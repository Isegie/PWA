...
if (isset($_POST['prijava'])) {
   ...
   $stmt = mysqli_stmt_init($dbc);
    ...
    if (password_verify($_POST['lozinka'], 
    $lozinkaKorisnika) && mysqli_stmt_num_rows($stmt) > 0) 
    { $uspjesnaPrijava = true;
      ...
      if($levelKorisnika == 1) 
      {
          ...
          (isset($_SESSION['$username'])) && $_SESSION['$level'] == 0) 
          { 
 ...
 while($row = mysqli_fetch_array($result))
  { 
     ...
     </div> </div> <div class="form-item"> <label for="about">Kratki sadržaj vijesti (do 50 znakova):</label> 
     <div class="form-field"> 
     <textarea name="about" id="" cols="30" rows="10" class="form-field-textual">'.$row['sazetak'].'</textarea>
      </div> </div> <div class="form-item"> <label for="content">Sadržaj vijesti:</label>
       <div class="form-field">
     ...
     <input type="file" class="input-text" id="pphoto" value="'.$row['slika'].'" name="pphoto"/> <br><img src="' . "img/" . $row['slika'] . '" width=100px> 
     </div> </div>
      <div class="form-item"> 
     <label for="category">Kategorija vijesti:</label>
      ...
      <select name="category" id="" class="form-field-textual" value="'.$row['kategorija'].'">
       ...
       <option value="politique">Politika</option>
        ...
        if($row['arhiva'] == 0)
         { 
             ...
             echo '</div> </label> </div> </div> <div class="form-item">
              <input type="hidden" name="id" class="form-field-textual" value="'.$row['id'].'">
               <button type="reset" value="Poništi">Poništi</button>
                ...
                if(isset($_POST['delete'])){
                   $id=$_POST['id'];
                     $query = "DELETE FROM vijesti WHERE id=$id "; $result = mysqli_query($dbc, $query); }
                ...
                $target_dir = 'img/'.$picture; move_uploaded_file($_FILES["pphoto"]["tmp_name"], $target_dir);
                $id=$_POST['id']; 
            ...
            echo '<p>Bok ' . $_SESSION['$username'] . '! Uspješno ste prijavljeni, ali niste administrator.</p>'; }
               ...
               echo' <script type="text/javascript"> //javascript validacija forme </script>';
               }
